...
const reducer = (state: CartStateType , action: ReducerAction): CartStateType => {
    switch(action.type) {
       ...
       const itemExsists: CartItemType | undefined = state.cart.find(item => item.sku === sku) 
       ...
       return {
        ...
        default: 
        throw new Error('Undidentified reducer action type')
    }
}
...
const useCartContext = (initCartState : CartStateType) => {
    ...
    const totalItem: number = state.cart.reduce((perviousValue, cartItem) => {
        return perviousValue + cartItem.qty
    }, 0)
